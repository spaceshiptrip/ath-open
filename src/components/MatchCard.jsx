  ...
  return (
    <div className={`card text-sm ${match.isMix ? 'border-l-4 border-ball' : ''}`}>
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
        {/* Team A pair */}
        <div className={`text-center p-2 rounded-lg ${match.winner === 'A' ? 'bg-blue-50 ring-1 ring-blue-300' : 'bg-gray-50'}`}>
          <p className="text-xs text-blue-600 font-semibold mb-0.5">Team A</p>
        ...
        {/* Team B pair */}
        <div className={`text-center p-2 rounded-lg ${match.winner === 'B' ? 'bg-red-50 ring-1 ring-red-300' : 'bg-gray-50'}`}>
          <p className="text-xs text-red-600 font-semibold mb-0.5">Team B</p>
      ...
      <div className="mt-3 flex items-center justify-between">
        {winnerLabel ? (
          <span className={`text-xs font-bold px-2 py-1 rounded-full ${
            match.winner === 'A' ? 'bg-blue-100 text-blue-800' : 'bg-red-100 text-red-800'
          }`}>
            ✓ {winnerLabel}
          </span>
        ) : (
